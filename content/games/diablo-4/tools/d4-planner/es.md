---
title: "D4 Planner"
description: "Build planner web focused en visualización del skill tree y paragon board de D4. Sharing via link permanente y comparativa side-by-side entre builds para hardcore theorycrafters."
quickTake: "El planner técnico para serios. Si quieres mockear paragon glyph placement antes de respecar in-game o comparar tu build vs un meta build pixel by pixel, esta es la opción. UI menos casual que Maxroll's planner pero más poder."
---
...
Free, sin login para usar (login para guardar permanente).

## Qué problema resuelve

Maxroll y D4builds tienen planners embedded pero limitados (no comparativa side-by-side, no rotation completa de paragon boards). Para users que necesitan precision máxima — theorycrafters serios, hardcore raiders pushing Pit tiers, content creators armando guides — D4 Planner provee el control granular.

También resuelve el problema de "respec cost": en D4 cambiar paragon allocations cuesta gold significativo en endgame. Planning en planner gratis antes de commit in-game ahorra mucho.

## Diferenciación vs D4builds.gg
...
- **Casuales sin endgame**: si juegas campaign casual, no necesitas planning de paragon depth.
- **Quien busca community discovery**: este es solo planner. Para discover builds existentes, D4builds.gg.
- **Quien quiere guidance editorial**: D4 Planner no opina. Si quieres "es S tier o no", Maxroll/Mobalytics.
- **Mobile-first users**: UI funciona en mobile pero la complejidad del paragon board es difícil en pantalla chica.
...
**Maintenance dependiente del owner**: planner mantenido por individuos, sin team profesional. Cuando Blizzard hace big patches, puede haber lag en updates.

**Sin integration con account**: no puedes importar tu gear actual del juego (D4 no expone API). Manual input only.

**UI puede sentirse rígida**: workflows de drag-and-drop en paragon a veces fight back. Acostumbrarse toma horas.

## Cómo empezar

1. [d4planner.io](https://d4planner.io). New Build.

2. Selecciona tu clase. Familiarizate con tabs.

3. Construí tu build current (basado en un Maxroll guide). Esto te ayuda a learning the planner UI.
