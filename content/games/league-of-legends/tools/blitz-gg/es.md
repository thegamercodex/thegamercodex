...
quickTake: "Si querés una sola tool que automatice todo el flujo —desde champ select hasta post-game— Blitz es la opción más ambiciosa. A cambio aceptás un cliente desktop pesado, paywall agresivo y telemetría amplia. Para usuarios que valoran convenience por encima de control fino."
---
...
**Auto-import de builds y runas a un click**: caso de uso central. Durante champ select, Blitz detecta tu pick y muestra build/runas; con un botón importás directo al cliente, sin copiar manualmente.
...
- **Preferís web puro sin instalar cliente desktop** → OP.GG cubre match history y U.GG cubre builds desde el navegador, sin runtime extra ni telemetría de la app.
- **Solo querés ayuda específica en champ select** → Porofessor es más liviano y especializado, sin el resto del feature creep.
- **Querés coaching estructurado y mejora a largo plazo** → Mobalytics tiene el GPI con framework de improvement; Blitz hace post-game analysis pero más superficial.
- **Te molesta la telemetría amplia o no querés depender de Pro tier para features avanzadas** → el modelo de monetización de Blitz es notablemente más empujado que op.gg / Porofessor.

## Cómo se usa en la práctica

1. Vas a `blitz.gg` y descargás la desktop app (Windows o macOS).

2. Instalás y permitís que se integre con el cliente de LoL. La primera vez te pide login (Riot ID o cuenta Blitz).

3. La app corre en background. Cuando abrís el cliente de LoL y entrás a champ select, Blitz dispara automáticamente el overlay con build y runas para tu pick.

4. Click en "Import" envía build y runas directo al cliente — no tenés que copiar manualmente ni cambiar de ventana.

5. Durante la partida, podés invocar el overlay para ver stats actuales y recomendaciones de items según el matchup.
...
Configuración recomendada: dejá la desktop app en startup para que esté disponible sin intervención manual.
...
1. Descargá la desktop app desde `blitz.gg` (Windows o macOS).

2. Instalá y aceptá los permisos de integración con el cliente de LoL. Login con Riot ID.

3. La primera vez que abras champ select, Blitz dispara overlay automáticamente. Probá importar runas y build a un click para validar la integración.

4. Configurá la app para arranque con el sistema si planeás usarla en cada sesión — eso elimina toda fricción manual.

5. Si después de 1-2 semanas sentís que la automation justifica la complejidad, considerá Pro tier. Si sentís que el overlay molesta más de lo que ayuda, desinstalala y volvé a tools especializadas (op.gg + U.GG + Porofessor cubren funciones equivalentes con menos overhead).
