...
quickTake: "El parser de demos más visual del ecosistema CS2. Si tu cuello de botella es utility usage — sabés que tirás malos smokes pero no entendés cuáles ni cuándo — esta es la tool específica."
---

## Qué es

scope.gg es una plataforma de análisis de demos para CS2 con énfasis en visualización 2D del map en tiempo de replay. Subís tus demos (o las trae automáticamente vía integraciones) y te muestra cada round desde una vista cenital tipo "minimapa expandido", reproduciendo posiciones de cada jugador, kills, asistencias, y crucialmente: cada smoke, flash y molotov lanzado, su trayectoria y dónde aterrizó.
...
Utility es una de las dimensiones más difíciles de evaluar en CS2 sin demo viewer dedicado. Sabés que tirás muchas granadas pero no sabés cuántas fueron efectivas. Sabés que algunos rounds caen mal pero no asociás el momento del smoke con la pérdida.

scope.gg te da exactamente eso: cada utility lanzada, su timing, su efectividad medida (cuántos enemigos cegó una flash, cuánta área bloqueó un smoke, cuánta lifebar quitó un molotov). Y lo reproduce visualmente, así no es un número abstracto sino un evento concreto que podés ver y entender.
...
**Aprender utility de pros**: scope.gg tiene cobertura de matches profesionales recientes — podés cargar el demo de tu pro favorito y comparar tus smokes con los suyos en el mismo mapa.
...
**Análisis de equipo**: sumás teammates y ves coordinación de utility — flashes lanzadas en simultaneous, smokes superpuestas, molotovs redundantes.
...
- **Reemplazo del cliente para review profundo de aim duels** → para review pixel-perfect de un duelo, abrí la demo en el cliente.

## Cómo se usa en la práctica

1. Vas a `scope.gg` y registrás cuenta (Steam OAuth).

2. Conectás Faceit o subís demos manualmente. Como Leetify, Faceit es la integración más fluida; matches de Premier/MM requieren upload.
...
**Tier free razonable pero limitada**: podés usar la mayoría de features pero el historial está capeado y comparativas avanzadas requieren pago.
...
2. Conectar Faceit si tenés. Si no, upload manual de demos desde la carpeta `replays`.

3. Procesar 3-5 matches y abrí uno en el replay 2D — el primer "ah, ahí está el smoke que se quemó" suele ser revelador.
