...
quickTake: "El más cercano a un coach personal asíncrono que existe para CS2. Si querés feedback accionable sin tener que pagar a un coach humano, este es el camino — pero la tier paga es donde está el valor real."
---

## Qué es

Leetify es una plataforma de análisis de performance para CS2 que combina automated demo parsing con AI scoring para entregar feedback estructurado. Subís tus demos (o las trae automáticamente vía Faceit/Matchmaking integration) y devuelve scores en categorías clave: aim, utility usage, positioning, opening duels, trade efficiency.
...
Mejorar en CS2 a partir de un punto requiere identificar errores específicos y patterns. Pero ver tus propios demos es tedioso, sesgado (no notás errores que no entendés que son errores), y caro (un coach humano cuesta $30-100 USD/hora).
...
**Identificar weak areas**: el dashboard muestra tu score por categoría y tu delta vs rank superior. Si tu utility usage es notablemente bajo, sabés dónde invertir tiempo de práctica.
...
**Comparar con teammates**: sumás a tu duo o equipo y ves head-to-head en categorías. Útil para conversaciones honestas tipo "vos morís en early round mucho más que yo".
...
- **Stats de matchmaking simple** → si solo querés KDA + win rate, CS Stats es más liviano y free.
...
1. Vas a `leetify.com` y registrás cuenta (Steam OAuth o email).

2. Conectás Faceit (si jugás ahí) — tus demos se importan automáticamente. Para Matchmaking de Valve, podés subir demos manualmente o instalar el desktop client (Windows) que las detecta.
...
5. Si activás Pro, podés cruzar dimensiones (mapa específico × arma × situación táctica) — esto es donde la tool entrega valor real.
...
**AI scores son orientativos, no verdad absoluta**: los algoritmos miran métricas cuantificables (timing, accuracy, utility damage) pero no entienden context táctico complejo (ej: morirías 1v1 a propósito para que tu compañero gane info). Tomá los scores como pulse, no como veredicto.

**Demos de Premier/MM tienen menos cobertura que Faceit**: el parser funciona mejor con demos de Faceit (formato más limpio, metadata más rica). Si jugás solo Premier o MM, la experiencia es más limitada.

**No hay versión mobile-first**: la web responde en mobile pero el flujo está pensado para desktop. Para review serio necesitás monitor.
...
2. Conectar Faceit si tenés cuenta — esto es lo que más rápido llena tu dashboard.
...
5. Si después de 2-3 semanas sentís que el feedback es accionable, considerar la tier Pro. Si no, las alternativas más livianas (CS Stats, scope.gg) probablemente cubren lo que necesitás.
