...
quickTake: "Si sos raid leader y querés saber por qué wipeó la raid en lugar de quién hizo poco DPS, Wipefest es la pestaña que abrís primero. Convierte el log en un boletín de errores de mecánica navegable jugador por jugador."
---
...
Para guilds en progresión Mythic, la separación entre WoWAnalyzer (qué hacés mal vos) y Wipefest (qué hace mal la raid) suele ser muy clara: una mira espejos individuales, la otra mira la ejecución colectiva.
...
**Validar strategy changes**: probás una nueva strat para una mecánica (ej: cambiar quién dispel a Comatose Curse). Comparás dos pulls en Wipefest y ves objetivamente si la coverage mejoró.
...
- **Players que no son raid leaders ni officers**: si no estás liderando, los datos de Wipefest a veces son más detalle del que querés. Para review propio individual, WoWAnalyzer es más directo.

## Cómo se usa en la práctica

1. Tu guild raidea y suben log a Warcraft Logs.

2. Copiás el link al reporte.

3. Lo pegás en wipefest.gg en la home page (o navegás directamente: `wipefest.gg/report/{report-id}`).

4. Wipefest carga la lista de bosses de ese raid. Click en el que querés analizar.
...
5. Aparece la página del encounter con tabs:
   ...
   - **Comparisons**: si tenés varios pulls del mismo boss, comparación lado a lado.

6. Para review post-wipe rápido: abrís Events, ordenás por damage taken o death, y reconstruís qué mecánica falló cerca del wipe.

7. Para coaching individual: abrís Players, elegís un personaje, y ves la lista de eventos donde tomó damage evitable.
...
**Curva de lectura**: los nombres de eventos vienen del log, que usa los nombres internos de las habilidades. La primera vez que abrís un boss nuevo, hay que aprender qué mecánica es cuál.
...
1. Abrí [wipefest.gg](https://www.wipefest.gg).

2. Pegá el link a tu reporte público de Warcraft Logs.

3. Elegí un boss donde tu raid haya wipeado. Esos son los más informativos para empezar a entender la herramienta.

4. Andá a tab Events, ordená por timestamp, y reconstruí los segundos previos al wipe.

5. Si sos raid leader, hacelo parte del flujo post-raid: 10-15 minutos en Wipefest después de cada noche de progresión.
